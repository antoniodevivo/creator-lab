...
export class Pipeline{
 constructor(root,keys,{groqRpm=20,transcriptionProvider='groq',fireworksRpm=60,classifier='jev'}={}){this.root=root;this.classifier=classifier;this.keys=keys;this.jobs=new Map();this.active=new Set();this.listeners=new Set();this.writes=new Map();if(!providers.transcriptionProviders[transcriptionProvider])throw new Error('Unsupported transcription provider');this.transcriptionProvider=transcriptionProvider;this.fireworksRpm=fireworksRpm;this.fireworksPacer=providers.createRequestPacer(fireworksRpm);this.groqRpm=groqRpm;this.groqPacer=providers.createRequestPacer(groqRpm);}
 ...
 async run(id){
  ...
  const k=this.keys();const provider=this.transcriptionProvider;const providerName=providers.transcriptionProviders[provider].name;const missing=[];if(!j.imported&&!j.posts.length&&!k.apify)missing.push('Apify');if(j.posts.some(p=>!p.transcript)||(!j.posts.length&&!j.imported)){if(!k[provider])missing.push(providerName);}if(this.classifier==='jev'&&!k.jev)missing.push('Jev');if(missing.length)throw new Error(`Connect ${missing.join(', ')} in Connections first`);
  ...
  const work=async()=>{try{
   ...
   const worker=async()=>{while(cursor<pending.length&&!j.pauseRequested){const p=pending[cursor++];p.error=null;
    try{
     ...
     if(p.transcript.text.trim().split(/\s+/).length<6){p.excludedReason='Too little spoken content for script analysis';p.status='no_speech';this.event(j,`Skipped short speech: ${p.id}`);await this.save(j);continue;}p.status='classifying';await this.save(j);const key=cacheKey(p.transcript,this.classifier);const cached=await this.readCache(key);p.analysis=cached||await providers.classify(p.transcript,k.jev,()=>{j.retries++;},{classifier:this.classifier});if(cached)p.analysis={...cached,reused:true};else await this.writeCache(key,p.analysis);
