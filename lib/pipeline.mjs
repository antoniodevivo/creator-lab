...
// Chosen per run in the New analysis form. Laya runs locally on the processor or, where available, on CUDA;
// Jev needs a TypeSafe key.
export const CLASSIFIERS=['laya','laya-cuda','jev'];
export const DEFAULT_CLASSIFIER='laya';
...
export class Pipeline{
 ...
 // Runs created before the classifier was selectable were classified by Jev.
 classifierFor(j){return j.classifier||'jev';}
 ...
 async create(config,rows){
  ...
  // Each run keeps the classifier it was created with, so resuming never mixes Laya and Jev labels.
const classifier=config.classifier||DEFAULT_CLASSIFIER;if(!CLASSIFIERS.includes(classifier))throw new Error('Choose Laya or Jev as the classifier');if(!providers.availableClassifiers().includes(classifier))throw new Error('Laya on CUDA is not available on this machine');
  const job={id:randomUUID(),creator:config.creator,limit,concurrency,budget,language,classifier,status:'ready',createdAt:new Date().toISOString(),schemaVersion:VERSION,posts:rows?deduplicate(rows.slice(0,1000)):[],imported:Boolean(rows),scrape:null,events:[],costs:{apify:null},error:null,elapsedMs:0,retries:0};
 ...
 async run(id){
  ...
  const k=this.keys();const provider=this.transcriptionProvider;const providerName=providers.transcriptionProviders[provider].name;const missing=[];if(!j.imported&&!j.posts.length&&!k.apify)missing.push('Apify');if(j.posts.some(p=>!p.transcript)||(!j.posts.length&&!j.imported)){if(!k[provider])missing.push(providerName);}if(this.classifierFor(j)==='jev'&&!k.jev)missing.push('Jev');if(missing.length)throw new Error(`Connect ${missing.join(', ')} in Connections first`)
if(((!j.posts.length&&!j.imported)||j.posts.some(p=>!p.transcript&&!p.excludedReason))&&!providers.ffmpegAvailable())throw new Error('FFmpeg is missing: run npm install (it bundles ffmpeg and ffprobe) or install FFmpeg on PATH, restart the server, then resume');
  ...
  const work=async()=>{try{
   ...
   const worker=async()=>{while(cursor<pending.length&&!j.pauseRequested){const p=pending[cursor++];p.error=null;
    try{
     if(!p.transcript){p.status='transcribing';await this.save(j);const cached=await this.readCache(`transcript-${p.id}`);p.transcript=cached||await providers.transcribe(p,k[provider],join(this.root,'tmp'),{provider,language:j.language,pacer:provider==='groq'?this.groqPacer:this.fireworksPacer,cancelled:()=>j.pauseRequested,onMedia:m=>{p.media=m;this.emit(j);},onRetry:()=>{j.retries++;this.event(j,`${providerName} retry queued: ${p.id}`);this.emit(j);}});if(cached)p.transcript={...cached,reused:true};else await this.writeCache(`transcript-${p.id}`,p.transcript);p.status='transcribed';this.event(j,`Transcribed ${p.id}`);await this.save(j);}
     if(p.transcript.text.trim().split(/\s+/).length<6){p.excludedReason='Too little spoken content for script analysis';p.status='no_speech';this.event(j,`Skipped short speech: ${p.id}`);await this.save(j);continue;}p.status='classifying';await this.save(j);const key=cacheKey(p.transcript,this.classifierFor(j).replace(/^laya-cuda$/,'laya'));const cached=await this.readCache(key);p.analysis=cached||await providers.classify(p.transcript,k.jev,()=>{j.retries++;},{classifier:this.classifierFor(j)});if(cached)p.analysis={...cached,reused:true};else await this.writeCache(key,p.analysis);
     p.status='complete';this.event(j,`Classified ${p.id}`);
    }catch(e){if(e instanceof providers.NoAudioTrackError){p.status='no_audio';p.media='no_audio';p.excludedReason=e.message;p.error=null;this.event(j,`Skipped media without audio: ${p.id}`);}else if(e instanceof providers.RunPausedError){p.status=p.transcript?'transcribed':'queued';p.error=null;}else{p.status='failed';p.error=e.message;this.event(j,`${p.id}: ${e.message}`);if([401,403,429].includes(e.status)){j.pauseRequested=true;j.error=e.message;}}}
    await this.save(j);
   }};
   // Thumbnails are a phase of their own, fetched alongside the speech work so the wall fills in early.
   const thumbs=j.posts.filter(p=>p.thumbnailUrl&&p.thumbnail!=='ready');let next=0;
   const thumbWorker=async()=>{while(next<thumbs.length&&!j.pauseRequested){const p=thumbs[next++];p.thumbnail=await this.fetchThumbnail(p);this.emit(j);}};
   await Promise.all([...Array.from({length:j.concurrency},worker),...Array.from({length:THUMBNAIL_CONCURRENCY},thumbWorker)]);
 ...
 thumbnailPath(p){return join(this.root,'media',p.id+'.img');}
 // Same file the /media route serves; returns the post's thumbnail state for progress.
 async fetchThumbnail(p){
  const file=this.thumbnailPath(p);
  try{await access(file);return 'ready';}catch{}
  try{const result=await providers.download(p.thumbnailUrl,8*1024*1024);if(!/^image\/(jpeg|png|webp)/.test(result.type))throw new Error('Unsupported thumbnail format');await writeFile(file,result.bytes);return 'ready';}
  catch{return 'failed';}
 }
