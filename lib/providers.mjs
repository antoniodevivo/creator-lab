...
import {classifyWithLaya,checkLaya,cudaAvailable} from './laya.mjs';
import net from 'node:net';
import {FFMPEG,FFPROBE} from './ffmpeg.mjs';
// Node tries IPv6 and IPv4 in parallel but gives each connection attempt only 250 ms. Where IPv6 is
// unreachable (e.g. WSL2) and the IPv4 CDN edge is a few hundred ms away, every attempt then fails with
// ETIMEDOUT and media downloads report 'fetch failed'. A longer attempt window keeps the fallback working.
net.setDefaultAutoSelectFamilyAttemptTimeout(2500);
// Laya on the processor always works; Laya on CUDA only where the GPU runtime is installed.
export const availableClassifiers=()=>['laya',...(cudaAvailable()?['laya-cuda']:[]),'jev'];
...
// Audio extraction needs both tools. Checked once per process, before a run starts, so a missing install
// stops the run with one clear message instead of failing every Reel.
let ffmpegChecked=null;
export function ffmpegAvailable(){ffmpegChecked??=[FFMPEG,FFPROBE].every(cmd=>spawnSync(cmd,['-version'],{stdio:'ignore'}).status===0);return ffmpegChecked;}
export async function download(raw,maxBytes=80*1024*1024){
 let url=mediaURL(raw);
 for(let i=0;i<4;i++){
  // 'fetch failed' alone hides the reason; keep the network code and host so a run log is actionable.
  const r=await fetch(url,{redirect:'manual',signal:AbortSignal.timeout(60000)}).catch(e=>{throw new Error(`Media download failed (${e.cause?.code||e.name||e.message}) from ${new URL(url).host}`,{cause:e});});
...
// onMedia reports the video stage for progress: 'downloaded' when the media was fetched and its audio extracted
// locally, 'remote' when the speech provider fetched the video URL itself.
export async function transcribe(post,key,tmpDir,{provider='groq',language='',onRetry=()=>{},pacer,cancelled=()=>false,onMedia=()=>{}}={}){
 ...
 let raw;
 if(provider==='fireworks'){prepared=await prepareAudio(post,tmpDir,{cancelled});onMedia('downloaded');raw=await send(prepared.audio);}
 else try{raw=await send();onMedia('remote');}catch(e){
  if(!(e instanceof ProviderError)||![400,413,422,504].includes(e.status))throw e;
  prepared=await prepareAudio(post,tmpDir,{cancelled});onMedia('downloaded');raw=await send(prepared.audio);
 }
 // Empty text is a valid result (music-only Reel): keep and cache it so the pipeline excludes the Reel
 // as too little speech instead of failing it, and a resume does not pay to transcribe it again.
 if(typeof raw.text!=='string')throw new Error('Transcription response has no text');
...
export async function classify(transcript,key,onRetry=()=>{},{classifier='jev'}={}){
 if(classifier==='laya')return classifyWithLaya(transcript,'cpu');
 if(classifier==='laya-cuda')return classifyWithLaya(transcript,'cuda');
...
export async function checkProvider(name,key){
 if(name==='laya')return checkLaya('cpu');
 if(name==='laya-cuda')return checkLaya('cuda');
