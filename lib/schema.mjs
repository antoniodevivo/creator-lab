...
// Jev keys keep their original shape so existing caches stay valid; other classifiers get their own entries.
export function cacheKey(transcript,classifier='jev'){return createHash('sha256').update(JSON.stringify({version:VERSION,...(classifier==='jev'?{}:{classifier}),...buildRequest(transcript)})).digest('hex');}
export function parseResult(raw,request){
 ...
 return {schemaVersion:VERSION,model:raw.model,labels,anatomy,opening:request.state.opening,inputTokens,costUsd:inputTokens===null?null:inputTokens*0.042/1e6,review:Object.values(labels).some(a=>a.confidence<0.65||a.value==='unclear')};
}
