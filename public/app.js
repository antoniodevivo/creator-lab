...
function renderConnections(){const active=['apify',boot.transcriptionProvider||'groq',boot.classifier==='jev'?'jev':'laya'];const count=active.filter(k=>boot.connections[k]?.configured).length;$('#connection-status').textContent=`${count}/3 connections ready`;$('#classifier-name').textContent=boot.classifier==='jev'?'Jev':'Laya';for(const k of ['apify','groq','fireworks','jev','laya'])$(`#${k}-status`).textContent=k==='laya'?(boot.classifier==='jev'?'· not used':boot.connections.laya?.verified?'· model loaded':'· local, no key'):boot.connections[k]?.verified?'· key verified':boot.connections[k]?.configured?'· configured':active.includes(k)?'· needed':'· optional';$('#transcription-provider').textContent=title(boot.transcriptionProvider||'groq');}
