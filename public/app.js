...
const classifierTitle=c=>c==='jev'?'JEV':'LAYA';
function readStats(){
...
function renderConnections(){const active=['apify',boot.transcriptionProvider||'groq',$('#classifier-select').value];const count=active.filter(k=>boot.connections[k]?.configured).length;$('#connection-status').textContent=`${count}/3 connections ready`;const select=$('#classifier-select');select.options[1].textContent=`Jev · TypeSafe API key${boot.connections.jev?.configured?'':' (not configured)'}`;for(const k of ['apify','groq','fireworks','jev','laya'])$(`#${k}-status`).textContent=k==='laya'?(boot.connections.laya?.verified?'· model loaded':'· local, no key'):boot.connections[k]?.verified?'· key verified':boot.connections[k]?.configured?'· configured':active.includes(k)?'· needed':'· optional';$('#transcription-provider').textContent=title(boot.transcriptionProvider||'groq');}
...
$('#classifier-select').onchange=()=>renderConnections();
