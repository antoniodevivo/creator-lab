...
const classifier=(localEnv.CLASSIFIER||process.env.CLASSIFIER||'laya').toLowerCase();if(!['laya','jev'].includes(classifier))throw new Error('CLASSIFIER must be laya or jev');
...
const pipeline=await new Pipeline(process.env.LAB_DATA_DIR||join(ROOT,'data'),keys,{transcriptionProvider,fireworksRpm:Number(localEnv.FIREWORKS_REQUESTS_PER_MINUTE||process.env.FIREWORKS_REQUESTS_PER_MINUTE||60),groqRpm:Number(localEnv.GROQ_REQUESTS_PER_MINUTE||process.env.GROQ_REQUESTS_PER_MINUTE||20),classifier}).init();
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(path==='/api/connections/check'&&req.method==='POST'){const results=Object.fromEntries(await Promise.all(Object.entries({...keys(),...(classifier==='laya'?{laya:'local'}:{})}).map(async([name,key])=>[name,await checkProvider(name,key)])));for(const [name,r]of Object.entries(results))verified[name]=r.verified;json(res,200,results);return;}
