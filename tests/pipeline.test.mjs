...
test('music-only Reels are excluded as too little speech, not failed, and not transcribed twice',async()=>{
 const root=await mkdtemp(join(tmpdir(),'creator-lab-test-'));const realFetch=global.fetch;let groq=0;global.fetch=async url=>{if(String(url).includes('groq.com')){groq++;return Response.json({text:'',segments:[],duration:20});}throw new Error('Unexpected request');};
 try{const rows=[{id:'music',ownerUsername:'tester',videoPlayCount:1000,likesCount:10,videoUrl:'https://scontent.cdninstagram.com/video.mp4',videoDuration:20}];
 const p=await new Pipeline(root,()=>({groq:'test',jev:'test'})).init();const run=await p.create({creator:'tester',limit:1,classifier:'jev'},rows);await p.run(run.id);while(p.active.size)await new Promise(r=>setTimeout(r,5));await p.writes.get(run.id);
 assert.equal(run.posts[0].status,'no_speech');assert.ok(run.posts[0].excludedReason);assert.equal(run.posts[0].error,null);assert.equal(run.posts[0].analysis,null);assert.equal(groq,1);
 const second=await p.create({creator:'tester',limit:1,classifier:'jev'},rows);await p.run(second.id);while(p.active.size)await new Promise(r=>setTimeout(r,5));await p.writes.get(second.id);assert.equal(second.posts[0].status,'no_speech');assert.equal(groq,1);
...
test('import -> Groq -> Jev -> persisted results -> cache reuse',async()=>{
 ...
 try{const p=await new Pipeline(root,()=>({groq:'test',jev:'test'})).init();const rows=[{id:'sample',ownerUsername:'tester',videoViewCount:888888,likesCount:10,videoUrl:'https://scontent.cdninstagram.com/video.mp4',videoDuration:8}];const run=await p.create({creator:'tester',limit:1,classifier:'jev'},rows);await p.run(run.id);while(p.active.size)await new Promise(r=>setTimeout(r,5));assert.equal(run.status,'complete');assert.equal(run.posts[0].analysis.anatomy.length,2);assert.equal(groq,1);assert.equal(jev,1);assert.equal(run.posts[0].transcript.costUsd,10/3600*.04);await p.writes.get(run.id);
 const restored=await new Pipeline(root,()=>({groq:'test',jev:'test'})).init();assert.equal(restored.jobs.size,1);assert.equal(restored.jobs.get(run.id).posts[0].status,'complete');const second=await restored.create({creator:'tester',limit:1,classifier:'jev'},rows);await restored.run(second.id);while(restored.active.size)await new Promise(r=>setTimeout(r,5));await restored.writes.get(second.id);assert.equal(groq,1);assert.equal(jev,1);assert.equal(second.posts[0].analysis.reused,true);
 }finally{global.fetch=realFetch;await rm(root,{recursive:true,force:true});}
});
test('invalid keys pause with recoverable failure',async()=>{const root=await mkdtemp(join(tmpdir(),'creator-lab-test-'));const realFetch=global.fetch;global.fetch=async()=>new Response('',{status:401});try{const p=await new Pipeline(root,()=>({jev:'bad'})).init();const j=await p.create({creator:'tester',classifier:'jev'},[{id:'bad',creator:'tester',transcript:transcript.text}]);await p.run(j.id);while(p.active.size)await new Promise(r=>setTimeout(r,5));await p.writes.get(j.id);assert.equal(j.status,'paused');assert.match(j.posts[0].error,/401/);assert.equal(j.posts[0].analysis,null);}finally{global.fetch=realFetch;await rm(root,{recursive:true,force:true});}});
